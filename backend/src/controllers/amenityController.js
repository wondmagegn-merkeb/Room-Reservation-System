...
export const createAmenityHandler = async (req, res) => {
  try {
    ...
    if (existingAmenity) {
      return res.status(400).json({ error: "Amenity name must be unique" });
    }
    ...
    await logOperation(
      "CREATE",
      `Created Amenity: ${name}, Description: ${description}`,
      req.user.id
    );

    res.status(201).json({ message: "Amenity added successfully.", data: amenity });
  } catch (error) {
    if (error.code === "P2002") {
      return res.status(400).json({ error: "Amenity name must be unique" });
    }
...
export const getAmenityByIdHandler = async (req, res) => {
  try {
    ...
    await logOperation(
      "READ",
      `Fetched Amenity by ID: ${req.params.id}`,
      req.user.id
    );
...
export const updateAmenityHandler = async (req, res) => {
  try {
    ...
    await logOperation(
      "UPDATE",
      `Updated Amenity - ID: ${amenityId}. Changes: ${JSON.stringify({
        old: existingAmenity,
        new: updatedAmenity,
      })}`,
      req.user.id
    ...
    res.json({ success: true, data: updatedAmenity });
  } catch (error) {
    res
      .status(500)
      .json({ error: "Internal server error", details: error.message });
...
export const deleteAmenityHandler = async (req, res) => {
  try {
    ...
    await logOperation(
      "DELETE",
      `Deleted Amenity - ID: ${amenity.id}, Name: ${amenity.name}, Description: ${amenity.description}`,
      req.user.id
    );
